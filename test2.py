...
def test_ai_features():
    ...
    # Test for the new `execute/managed` endpoint
    print("\n" + "--- Testing Managed Prompt Execution ---")
    ...
    if not openai_api_key:
        ...
        response = run_test("Managed Execution (Success)", "POST", f"{BASE_URL}/prompts/execute", payload=exec_payload, expected_status=200)
        if response:
            assert "final_text" in response
            assert "Olympia" in response["final_text"]
            ...
            assert "cost" in response and response["cost"] >= 0
