...
def run_test(name, method, url, payload=None, expected_status=200, headers=None, exit_on_failure=True):
    ...
    try:
        ...
        # Re-raise the exception to be caught by the try...finally block
        raise e


# V-- THIS IS THE FINAL FIX: A ROBUST TEST STRUCTURE --V
def test_security_and_data_scoping():
    print_test_header("Security & Data Scoping")

    # A list to keep track of all prompts created during the test
    prompts_to_delete = {"user_a": [], "user_b": []}
    prompt_a1, prompt_a2, prompt_b1 = None, None, None
    
    try:
        # 1. Test for Anonymous Data Leak (unchanged)
        ...
        run_test("Anonymous GET /prompts/", "GET", f"{BASE_URL}/prompts/", expected_status=401, headers=anon_headers, exit_on_failure=False)
        run_test("Anonymous GET /metrics/activity/recent", "GET", f"{BASE_URL}/metrics/activity/recent", expected_status=401, headers=anon_headers, exit_on_failure=False)
        run_test("Anonymous GET /metrics/prompts/starred", "GET", f"{BASE_URL}/metrics/prompts/starred", expected_status=401, headers=anon_headers, exit_on_failure=False)
        ...
        # If any assertion fails, print a failure message and allow 'finally' to run
        print(f"\n--- TEST FAILED: {e} ---")
        # Exit with a failure code after cleanup
        sys.exit(1)
        ...
        if prompts_to_delete["user_a"]:
            ...
            for prompt_id in prompts_to_delete["user_a"]:
                run_test(f"Delete User A Prompt {prompt_id}", "DELETE", f"{BASE_URL}/prompts/{prompt_id}", expected_status=204, exit_on_failure=False)
        
        if prompts_to_delete["user_b"]:
            HEADERS["Authorization"] = f"Bearer {get_auth_token(SECOND_TEST_USER_ID)}"
            for prompt_id in prompts_to_delete["user_b"]:
                run_test(f"Delete User B Prompt {prompt_id}", "DELETE", f"{BASE_URL}/prompts/{prompt_id}", expected_status=204, exit_on_failure=False)
# ^-- END OF FIX --^
