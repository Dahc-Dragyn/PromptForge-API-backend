...
# --- MODIFIED: Added `exit_on_fail` parameter ---
def run_test(name, method, url, payload=None, expected_status=None, exit_on_fail=True):
    ...
    try:
        ...
        # --- MODIFIED: Respect `exit_on_fail` flag ---
...
def test_prompt_endpoints():
    ...
    run_test("Get Single Prompt (No Slash)", "GET", f"{BASE_URL}/prompts/{prompt_id}") # This is correct for prompts
...
# --- MODIFIED: Added diagnostic tests for Get Single Template ---
def test_template_and_sandbox_endpoints():
    ...
    # --- MODIFIED: Capture the created template to get its ID ---
    ...
    # --- NEW: Diagnostic tests for the broken "View" button endpoint ---
    print("\n--- Running Template Detail Endpoint Diagnostics (Testing 'View' button) ---")
    
    # Test 1: The route suggested by the Changeover Brief (no slash)
    # We expect this to fail with a 500 Internal Server Error based on logs
    run_test(
        name="Get Single Template (No Slash)", 
        method="GET", 
        url=f"{BASE_URL}/templates/{template_id}", 
        expected_status=200,
        exit_on_fail=False # <-- Set to False to log the error and continue
    )
    
    # Test 2: The route that is consistent with all other template endpoints (with slash)
    # We expect this to fail with a 404 Not Found based on logs
    run_test(
        name="Get Single Template (With Slash)", 
        method="GET", 
        url=f"{BASE_URL}/templates/{template_id}/", 
        expected_status=200,
        exit_on_fail=False # <-- Set to False to log the error and continue
    ...
    try: # Indentation fixed
        ...
        if all_templates:
            for t in all_templates:
                if "test_script" in t.get("tags", []): # Indentation fixed
...
if __name__ == "__main__":
    ...
    try:
        ...
        test_ai_and_analysis_endpoints() # Indentation fixed
